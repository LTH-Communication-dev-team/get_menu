...
class FileMutationSlot implements \TYPO3\CMS\Core\SingletonInterface
{
    ...
    {
        //$GLOBALS['TYPO3_DB']->exec_INSERTquery('tx_devlog', array('msg' => $file->getIdentifier(), 'crdate' => time()));

        $this->flushCacheForFiles('http://www.lth.se/fileadmin' . $file->getIdentifier());
    }
    ...
    protected function flushCacheForFiles($identifier)
    {
        try {
            $curl = curl_init($identifier);
            curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
            curl_setopt($curl, CURLOPT_CUSTOMREQUEST, "PURGE");
            $res = curl_exec($curl);
            //$GLOBALS['TYPO3_DB']->exec_INSERTquery('tx_devlog', array('msg' => $res, 'crdate' => time()));
        } catch(Exception $e) {
            //$GLOBALS['TYPO3_DB']->exec_INSERTquery('tx_devlog', array('msg' => $res, 'crdate' => time()));
        }
    }
